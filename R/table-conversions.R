...
#' @export
as.long_format_table.long_format_table = function(table, ...) {
  return(table)
}

#' @method as.long_format_table data.frame
#' @export
as.long_format_table.data.frame = function(
  table,
  fontName = "Roboto",
  fontSize = 8,
  alignment = "START",
...
#' @export
as.long_format_table.huxtable = function(
  table,
  fontName = "Roboto",
  fontSize = 8,
  ...
  ...
  tidy2 = tidy %>%
    ...
    dplyr::mutate(
      ...
      fontName = attr(table, "font") %>%
        as.vector() %>%
        tidyr::replace_na(fontName),
      fontSize = attr(table, "font_size") %>%
        as.vector() %>%
        tidyr::replace_na(fontSize),
      fillColour = attr(table, "background_color") %>%
        as.vector() %>%
        tidyr::replace_na("#FFFFFF"),
      bold = attr(table, "bold") %>% as.vector() %>% tidyr::replace_na(FALSE),
      italic = attr(table, "italic") %>%
        as.vector() %>%
      ...
      topBorderWeight = utils::head(attr(table, "tb_borders")$thickness, -1) %>%
        as.vector() %>%
        tidyr::replace_na(0),
      bottomBorderWeight = attr(table, "tb_borders")$thickness[-1, ] %>%
        as.vector() %>%
        tidyr::replace_na(0),
      leftBorderWeight = t(utils::head(
        t(attr(table, "lr_borders")$thickness),
        -1
      )) %>%
        as.vector() %>%
        tidyr::replace_na(0),
      rightBorderWeight = attr(table, "lr_borders")$thickness[, -1] %>%
        as.vector() %>%
        tidyr::replace_na(0),
      bottomPadding = attr(table, "bottom_padding") %>%
        as.vector() %>%
        tidyr::replace_na(0),
      topPadding = attr(table, "top_padding") %>%
        as.vector() %>%
        tidyr::replace_na(0),
      leftPadding = attr(table, "left_padding") %>%
        as.vector() %>%
        tidyr::replace_na(1),
      rightPadding = attr(table, "right_padding") %>%
        as.vector() %>%
        tidyr::replace_na(1)
    ) %>%
  ...
  attr(tidy2, "colWidths") = attr(table, "col_width") %>%
    tidyr::replace_na(1) %>%
    unname()
...
.remove_spans = function(tidy) {
  ...
  tidy %>%
    dplyr::anti_join(dups, by = c("row", "col")) %>%
    dplyr::left_join(spanEnds, by = c("row", "col"), suffix = c("", ".new")) %>%
    dplyr::mutate(
      bottomBorderWeight = ifelse(
        is.na(bottomBorderWeight.new),
        bottomBorderWeight,
        bottomBorderWeight.new
      ),
      rightBorderWeight = ifelse(
        is.na(rightBorderWeight.new),
        rightBorderWeight,
        rightBorderWeight.new
      )
    ) %>%
