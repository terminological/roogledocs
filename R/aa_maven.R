...
#' @param quiet don't report messages (defaults to `getOption("rmaven.quiet")`
#'   or TRUE)
#' @param max_heap optional. if a string like `"2048m"` the `-Xmx` option value
#'   to start the `JVM` - if a string like `"75%"` the `-XX:MaxRAMPercentage`,
...
#' @param ... other parameters ignored apart from `packaging`
...
#' @param settings_path the file path of the `settings.xml` to update (generally
#'   the supplied default is what you want to use)
#' @return the location of the maven repository
#' @keywords internal
.get_repository_location = function(settings_path = .settings_path()) {
  if (fs::file_exists(settings_path)) {
    settings_list = xml2::read_xml(settings_path) %>% xml2::as_list()
    repo = fs::path(settings_list$settings$localRepository[[1]])
  } else {
    # make sure a settings.xml exists
    # and return the default
    repo = .set_repository_location()
  }
  fs::dir_create(repo)
  return(repo)
}
...
#' local `.m2` repository. The local path is made available for importing it into
#' the `rJava` classpath for example.
#'
...
#' @param outputDirectory optional path, defaults to the `rmaven` cache
#'   directory
#' @param nocache normally artifacts are only fetched if required, `nocache`
#'   forces fetching
#' @param verbose how much output from maven, one of "normal", "quiet", "debug"
...
#' @param verbose how much output from maven, one of "normal", "quiet", "debug"
#'
#' @return a character vector of the `classpath` jar files (including the
...
#'   parameter
#'
#' @return the path to the compiled 'jar' file. If this is a fat jar this can be
